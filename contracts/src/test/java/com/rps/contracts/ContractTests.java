...
public class ContractTests {
    ...
    public void issuerAndRecipientCannotHaveSameEmail() {
        rpsState state = new rpsState("Hello-World",alice.getParty(),bob.getParty());
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                tx.input(rpsContract.ID, state);
                tx.output(rpsContract.ID, state);
                tx.command(alice.getPublicKey(), new rpsContract.Commands.Send());
