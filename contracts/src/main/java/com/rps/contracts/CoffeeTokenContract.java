...
public class CoffeeTokenContract implements Contract {
    public static String ID = "com.rps.contracts.TokenContract";

    @Override
    public void verify(LedgerTransaction tx) throws IllegalArgumentException {

        CommandWithParties<CoffeeTokenContract.Commands> command = requireSingleCommand(tx.getCommands(), CoffeeTokenContract.Commands.class);

        List<ContractState> inputs = tx.getInputStates();
        List<ContractState> outputs = tx.getOutputStates();

        if (command.getValue() instanceof CoffeeTokenContract.Commands.Issue) {
            requireThat(req -> {
                ...
                req.using("Output must be a TokenState.", outputs.get(0) instanceof CoffeeTokenType);
                CoffeeTokenType output = (CoffeeTokenType) outputs.get(0);
